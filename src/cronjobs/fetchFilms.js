...
export default async function fetchFilms () {
  try {
    ...
    const trailers = (await axios
      .all(newFilms.map((film) => axios.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=${film.title} trailer&maxResults=1&key=${YOUTUBE_API_KEY}`))))
